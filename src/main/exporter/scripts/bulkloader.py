...
def bulkloadData(type):
  ...
  if not(SchemaUtil.isTableExists(tablename)) :
    ...
    inittable = Pig.compileFromFile('load-balance-' + type , default_exporter_src + '/pig/bucket.pig')
...
if __name__ == "__main__":
  try:
    opts, args = getopt.getopt(sys.argv[1:],"hd:e:r:u:",["type=","exporter=","release=", "upload="])
